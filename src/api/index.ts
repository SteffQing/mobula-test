...
import { CONTRACT_ADDRESS, CREATION_BLOCK_NUMBER } from "../constants";
import {
  LIQUIDITY_DECIMALS,
  SRG_PRICE_DECIMALS,
...
setInterval(async () => {
  await Promise.all([
    redis.set("cachedVolume", memory.inMemoryVolume),
    redis.set("cachedLastBlock", memory.inMemoryLastBlock),
  ]);
